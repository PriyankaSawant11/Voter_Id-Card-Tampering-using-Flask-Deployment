...
def index():
	...
	if request.method == "POST":
                ...
                uploaded_image.save(os.path.join(app.config['INITIAL_FILE_UPLOADS'], 'image.jpeg'))
                ...
                original_image.save(os.path.join(app.config['EXISTNG_FILE'], 'image.jpeg'))

                # Read uploaded and original image as array
                original_image = cv2.imread(os.path.join(app.config['EXISTNG_FILE'], 'image.jpeg'))
                uploaded_image = cv2.imread(os.path.join(app.config['INITIAL_FILE_UPLOADS'], 'image.jpeg'))
